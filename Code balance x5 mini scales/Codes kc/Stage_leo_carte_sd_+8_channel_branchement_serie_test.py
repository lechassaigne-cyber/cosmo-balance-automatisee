...
# -------------------------
# communication mère & fille TCA9548A
# -------------------------

class NestedMuxChannel:
    def __init__(self, i2c, mother_addr=0x70, mother_channel=0, daughter_addr=0x70, daughter_channel=0):
        if mother_channel < 0 or mother_channel > 7:
            raise ValueError("Le canal mere doit etre entre 0 et 7")
        if daughter_channel < 0 or daughter_channel > 7:
            raise ValueError("Le canal fille doit etre entre 0 et 7")

        self._i2c = i2c
        self._mother_addr = mother_addr
        self._mother_channel = mother_channel
        self._daughter_addr = daughter_addr
        self._daughter_channel = daughter_channel

    def _select_path_locked(self):
        self._i2c.writeto(self._mother_addr, bytes((1 << self._mother_channel,)))
        self._i2c.writeto(self._daughter_addr, bytes((1 << self._daughter_channel,)))
...
DAUGHTER_TCA_ADDR = 0x70

def detect_daughter_mux_ports():
    states = {}
    tca_write_mask(0x00)
    time.sleep(0.02)
...
def probe_miniscale_on_selected_daughter(mother_ch, daughter_ch, addr=SCALE_ADDR):
    ok = False
    ...
    for _ in range(3):
        try:
            while not root_i2c.try_lock():
                pass
            try:
                # selection du chemin : mux mere -> mux fille
                root_i2c.writeto(MUX_ADDR, bytes((1 << mother_ch,)))
                root_i2c.writeto(DAUGHTER_TCA_ADDR, bytes((1 << daughter_ch,)))
...
def detect_scale_channels_on_daughter(mother_ch, addr=SCALE_ADDR):
    found = []
    tca_write_mask(0x00)
    time.sleep(0.02)

    for daughter_ch in range(8):
        if probe_miniscale_on_selected_daughter(mother_ch, daughter_ch, addr):
            found.append(daughter_ch)
...
def choose_daughter_mux_port():
    states = detect_daughter_mux_ports()

    ok_ports = [p for p in range(8) if states.get(p) == "OK"]
    sel = ok_ports[0] if ok_ports else 0

    while True:
        tm = time.localtime()

        title_lbl.text = "Detection MUX filles"
        mode_lbl.text = "Mode: SELECT MUX"
        ...
        for ch in range(8):
            marker = ">" if ch == sel else " "
            txt = "{}P{}:{}".format(marker, ch, states.get(ch, "---"))
        ...
        info_lbl.text = "OK = mux fille detecte"
        status_lbl.text = "B/PRESS valider  C rescanner"
        list_lbl.text = ""
        hint_lbl.text = "L/R ou U/D : choisir"

        if DB_L.update(pressed(JOY_LEFT)) or DB_U.update(pressed(JOY_UP)):
            sel = (sel - 1) % 8

        if DB_R.update(pressed(JOY_RIGHT)) or DB_D.update(pressed(JOY_DOWN)):
            sel = (sel + 1) % 8

        if DB_C.update(pressed(BTN_C)):
            states = detect_daughter_mux_ports()
            ok_ports = [p for p in range(8) if states.get(p) == "OK"]
            if ok_ports and sel not in ok_ports:
                sel = ok_ports[0]

        if DB_B.update(pressed(BTN_B)) or DB_P.update(pressed(JOY_PRESS)):
            state = states.get(sel, "---")
            if state == "OK":
                hint_lbl.text = "L/R:sel  A:tare  B:calib  C:set  Joy:log"
                status_lbl.text = "MUX fille P{} selectionne".format(sel)
                return sel
            else:
                status_lbl.text = "P{} non valide".format(sel)
...
class ScaleApp:
    def __init__(self):
        ...
        self.scales = []
        self.active_idx = -1
        self.selected_daughter_port = None
    ...
    def init_scales(self):
        self.selected_daughter_port = choose_daughter_mux_port()

        all_found = detect_scale_channels_on_daughter(self.selected_daughter_port)
        channels = all_found[:TARGET_COUNT] if len(all_found) >= TARGET_COUNT else all_found[:]

        self.rebuild_scales(channels)
        self.set_status(
            "MUX P{} : {} balance(s)".format(
                self.selected_daughter_port,
                len(self.scales)
            )
        )
