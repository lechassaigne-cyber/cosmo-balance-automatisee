...
def _mk_button(pin):
    ...
    b.pull = Pull.UP
    return b
#Interface utilisateur pour naviguer entre les balances, tarer, calibrer et lancer les actions
...
class MiniScaleEnhanced:
    ...
    def read_weight_raw(self) -> float:                                                  #Lecture du poids sur la balance, sans appliquer la tare logicielle
    ...
    def read_weight(self) -> float:                                                    #Lecture du poids sur la balance en appliquant les filtres anti-dérive et de stabilité
        self.read_count += 1
...
#  I2C + TCA
# =========================
root_i2c = busio.I2C(board.SCL, board.SDA, frequency=100000)   #Initialisation du bus I²C utilisé pour parler au TCA et aux balances


def detect_tca_addr(i2c):                                      #Recherche du multiplexeur TCA9548A sur le bus I²C
...
def detect_scale_channels(addr=0x26):                                #Active un seul canal du TCA pour isoler une balance donnée
    found = []
    tca_write_mask(0x00)
    time.sleep(0.02)
    for ch in range(8):                                              #Teste chaque canal du multiplexeur pour détecter quelles balances sont présentes
