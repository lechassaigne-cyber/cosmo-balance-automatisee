...
title_lbl = mk_label("Stage Leo - Scales x5", 6, 18, scale=2, color=0x00FFD0)
...
for widget in (title_lbl, mode_lbl, focus_lbl, adc_lbl, info_lbl, status_lbl, list_lbl, hint_lbl):
    group.append(widget)
    ...
    def update_display(self, now, snapshots):
        ...
        for e in self.scales:
            ...
            if not snap_i or "error" in snap_i:
                ...
                parts.append("S{}:{:.1f}{}".format(
                    ch_i,
                    snap_i["weight"],
                    "+" if snap_i["stable"] else ""
                ))
